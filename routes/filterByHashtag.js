...
module.exports = function (hashtag,res,path)
{	
	var hashtags;
							...
							if (err)
								...
								db.collection('tweets').distinct( 'tweet.entities.hashtags.text'  ,function(err, fhashtags)
						        {
									if (err)
										...
										res.render(path, { 'title': 'hashtags' , tweet_data: tweets , 'load_options':  filter_options , 'users': users_returned ,'hashtags': fhashtags.sort() , 'url': hashtags});
									}
						        });
